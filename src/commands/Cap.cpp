...
bool CapCommand::needsRegistration() const { return false; }
...
void CapCommand::execute(Server& server, Client& client, const Message& msg) {
    ...
    if (subcommand == "LS") {
        ...
        server.sendLine(
            client, ":" + server.serverName() + " CAP * NAK :" + msg.param(1));
    } else if (subcommand == "END") {
        return;
    } else {
        throw IrcException(Reply::ERR_INVALIDCAPCMD, client.nick(),
